let foo: { [index: string]: string } = {};

foo["en"] = `
<h2>Education</h2>
<br />
<div class="aboutWrapper">
  <img src="img/chief.png" alt="Illinois Chief Logo" />
  <ul>
    <li><b>University of Illinois at Urbana-Champaign</b></li>
    <br />
    <li class="onMobile">M.C.S. in Data Science</li>
    <li class="onPC">Master of Computer Science in Data Science</li>
    <li class="onMobile">B.Sc. in Accountancy (Honor)</li>
    <li class="onPC">Bachelor of Science in Accountancy (Honor)</li>
    <li class="onMobile">B.Sc. in Finance (Honor)</li>
    <li class="onPC">Bachelor of Science in Finance (Honor)</li>
    <li>Minor in Computer Science</li>
  </ul>
</div>
<br />
<hr />
<br />
<h2>Employment</h2>
<br />
<div class="aboutWrapper">
  <ul>
    <li>2021-Current</li>
  </ul>
  <ul>
    <li><b>HYUNDAI CAPITAL SERVICES – SENIOR DATA SCIENTIST, DATA SCIENCE TEAM </b></li>
    <li>Recommender System for Banner Ads on 3rd-Party Sites</li>
    <li>GTM-GA4-BigQuery Implementation</li>
  </ul>
</div>
<div class="aboutWrapper">
  <ul>
    <li>2017-2021</li>
  </ul>
  <ul>
    <li>
      <b>EY CONSULTING – SENIOR CONSULTANT, DATA &amp; ANALYTICS TEAM</b>
    </li>
    <li>
      Samsung Electronics – Development of E2E Global Sellouts Management
      Solution
    </li>
    <li>Samsung Electronics – SARIMAX Forecasting on Flagship Cell Phone Sellout</li>
    <li>Samsung Electronics – GIS Analysis for Market Expansion</li>
    <li>Samsung Electronics – Survival Analysis on Active Devices</li>
    <li>Samsung Electronics – Optimization of Salesforce Staffing</li>
    <li>Samsung Electronics – Semi-Realtime Monitoring of the SCM</li>
    <li>NH Investment and Securities – Personalized Product and Service</li>
    <li>Emart – Demand Forecasting of Fresh Foods</li>
  </ul>
</div>
<div class="aboutWrapper">
  <ul>
    <li>2014-2017</li>
  </ul>
  <ul>
    <li><b>HANWHA CHEMICAL CO. – MANAGER, FIANCIAL ACCOUNTING TEAM</b></li>
    <li>Financial Statement Disclosure & Automation of Footnote Disclosure</li>
    <li>Development of Fair-value Assessment Process</li>
  </ul>
</div>
<div class="aboutWrapper">
  <ul>
    <li>2012-2013</li>
  </ul>
  <ul>
    <li><b>GREEN LIGHT TRADING LLC – HIGH-FREQUENCY TRADER</b></li>
    <li>Data Mining of Trading Log</li>
    <li>Hyperparameter Tuning for HFT System</li>
  </ul>
</div>
<br />
<hr />
<br />
<h2>Skills</h2>
<br />
<div class="skillWrapper">
  <p>License</p>
  <ul>
  <li>Passed All of CFA Exams</li>
  </ul>
</div>
`;

foo["kr"] = `
<h2>학력 사항</h2>
<br />
<div class="aboutWrapper">
  <img src="img/chief.png" alt="Illinois Chief Logo" />
  <ul>
    <li><b>University of Illinois at Urbana-Champaign</b></li>
    <br />
    <li class="onMobile">컴퓨터 과학 석사 - 데이터 사이언스</li>
    <li class="onPC">컴퓨터 과학 석사 - 데이터 사이언스</li>
    <li class="onMobile">회계학 학사 (명예졸업)</li>
    <li class="onPC">회계학 학사 (명예졸업)</li>
    <li class="onMobile">금융학 학사 (명예졸업)</li>
    <li class="onPC">금융학 학사 (명예졸업)</li>
    <li>컴퓨터 과학 부전공</li>
  </ul>
</div>
<br />
<hr />
<br />
<h2>경력 사항</h2>
<br />
<div class="aboutWrapper">
  <ul>
    <li>2021-현재</li>
  </ul>
  <ul>
    <li><b>현대캐피탈 – 시니어 데이터 과학자, 데이터사이언스 팀 </b></li>
    <li>외부채널 배너 추천모델 개발 및 최적화</li>
    <li>GTM-GA4-BigQuery 도입 및 실무 적용</li>
  </ul>
</div>
<div class="aboutWrapper">
  <ul>
    <li>2017-2021</li>
  </ul>
  <ul>
    <li>
      <b>EY 컨설팅 – 시니어 컨설턴트, DATA &amp; ANALYTICS 팀</b>
    </li>
    <li>삼성전자 – E2E 글로벌 Sellouts 관제 솔루션 개발</li>
    <li>삼성전자 – 플래그쉽 휴대폰 Sellout SARIMAX 예측 모델 개발</li>
    <li>삼성전자 – 시장 확장 전략을 위한 GIS 상권 분석</li>
    <li>삼성전자 – 활성 휴대폰들에 대한 생존 분석</li>
    <li>삼성전자 – 체험매장 영업사원 배치 최적화</li>
    <li>삼성전자 – 준-실시간 SCM 모니터링 서비스 개발</li>
    <li>NH 투자증권 – 상품 및 서비스 개인화</li>
    <li>이마트 – 신선 카테고리 상품들의 수요 예측</li>
  </ul>
</div>
<div class="aboutWrapper">
  <ul>
    <li>2014-2017</li>
  </ul>
  <ul>
    <li><b>한화케미칼 – 매니저, 재무회계 팀</b></li>
    <li>재무제표 공시 및 주석 입력 자동화</li>
    <li>무형자산, 금융자산 공정가치 평가 프로세스 수립</li>
  </ul>
</div>
<div class="aboutWrapper">
  <ul>
    <li>2012-2013</li>
  </ul>
  <ul>
    <li><b>GREEN LIGHT TRADING LLC – HIGH-FREQUENCY TRADER</b></li>
    <li>매매로그 데이터 마이닝</li>
    <li>HFT 시스템 하이퍼 파라미터 튜닝</li>
  </ul>
</div>
<br />
<hr />
<br />
<h2>기술 사항</h2>
<br />
<div class="skillWrapper">
  <p>자격증</p>
  <ul>
  <li>모든 CFA 시험 통과</li>
  </ul>
</div>
`;

export default foo;
